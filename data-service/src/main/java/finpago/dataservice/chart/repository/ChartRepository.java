...
public interface ChartRepository extends JpaRepository<Chart, LocalDateTime> {
    ...
    List<Chart> findByStockTickerAndChartTypeOrderByReportDate(String stockTicker, String chartType);

    // 종목 코드 별, 차트 타입 별, reportDate가 startDate와 endDate 사이에 있는 데이터 조회 (reportDate 내림차순 정렬)
    List<Chart> findByStockTickerAndChartTypeAndReportDateBetweenOrderByReportDate(String stockTicker,
                                                                                       String chartType,
